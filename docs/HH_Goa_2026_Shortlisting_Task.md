# HH Goa 2026 Shortlisting Task: Build a Frame / ID Card Generator
...
2. (Format B only) User fills in a couple of quick fields: name, stack/role, etc. 
...
## Requirements
* **Speed:** Upload to finished result should be fast—a few seconds, not a loading screen. 
* **Handles real photos:** Portrait, landscape, off-center crops, different aspect ratios; don't assume users will crop first.
* **On-brand:** It should be unmistakably this event, not a generic badge with a logo pasted on.
* **Downloadable output:** A real image file, not something that only renders on-screen. 
* **Working share flow:** Pre-filled caption + hashtag `#FrameInGoa`. If you share via link rather than direct image attach, ensure the link preview (OG image) shows the generated graphic, not a blank/default thumbnail.
* **Mobile-friendly:** Most people will use this from their phone.
...
## Where to submit
Fill the form here: https://forms.gle/jM5hTaGvsrfEfixPA
